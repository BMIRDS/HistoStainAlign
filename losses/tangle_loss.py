...
def smooth_rank_measure(embedding_matrix, eps=1e-7):
    ...
    Copied from: https://github.com/mahmoodlab/TANGLE/blob/main/core/loss/tangle_loss.py
    """
...
def apply_random_mask(patch_embeddings, percentage):
    """
    Args:
        - patch_embeddings (torch.Tensor): Tensor of patch embeddings (batch_size, seq_len, dim_size).
        - percentage (float): Percentage of elements to mask.
    Returns:
        - masked_embeddings (torch.Tensor): Masked patch embeddings.
    """
    batch_size, _, embed_dim = patch_embeddings.shape

    masked_embeddings = patch_embeddings.clone()
    for i in range(batch_size):
        valid_indices = torch.arange(patch_embeddings[i].size(0))
        valid_count = len(valid_indices)
        mask_count = int(percentage * valid_count)

        # Create a random mask for the valid indices
        mask = torch.cat([torch.ones(mask_count), torch.zeros(valid_count - mask_count)]).bool()    # 'True' are the masked elements                                                           # convert to boolean tensor
        mask = mask[torch.randperm(valid_count)].to(patch_embeddings.device)                        # shuffle the mask

        full_mask = torch.zeros(valid_count, dtype=torch.bool, device=patch_embeddings.device)
        full_mask[valid_indices] = mask
        full_mask = ~full_mask                        # 'False' are the masked elements
        full_mask = full_mask.unsqueeze(0)
        binary_mask = full_mask.clone().squeeze(0)

        assert torch.sum(full_mask) == valid_count - mask_count, "Masking is not done correctly!"

        # Apply the mask to the embeddings
        full_mask = full_mask.unsqueeze(-1).expand(-1, full_mask.size(1), embed_dim)
        masked_embeddings[i] = patch_embeddings[i] * full_mask

        # assert that the masked embeddings are zero where the mask is True
        assert torch.sum(masked_embeddings[i][~binary_mask]) == 0, "Masking is not done correctly!"
...
class InfoNCE(nn.Module):
    ...
    Credit: Copied from https://github.com/mahmoodlab/TANGLE/blob/main/core/loss/tangle_loss.py
    """
